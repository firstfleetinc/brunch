...
'use strict';
const brunchdebug = debug('brunch:server');
// options types:
...
class BrunchServer {
    ...
    start() {
        brunchdebug(`starting server of type ${this.type}`);
        switch (this.type) {
    ...
    async customFileServer() {
        ...
        let server;
        try {
            server = await import(this.serverPath);
        ...
        brunchdebug(`Invoking custom startServer with: ${JSON.stringify(serverConfig)}`);
    ...
    defaultStaticServer() {
        ...
        return new Promise((resolve, reject) => {
            ...
            server.listen(port, hostname, () => {
                ...
                if (!ifshow)
                    logger.info(`app started on http://${hostname}:${port}/`);
                resolve(this);
            });
        });
    }
