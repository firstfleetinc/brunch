...
import speed from "since-app-start";
import { serve } from "./serve.js";
import { installDeps } from "deps-install";
import write from "./fs_utils/write.js";
import ignored from "./fs_utils/is_ignored.js";
import FileList from "./fs_utils/file_list.js";
import pipeline from "./fs_utils/pipeline.js";
import * as application from "./utils/config.js";
import helpers from "./utils/helpers.js";
const getRelativePath = sysPath.relative;
const brunchdebug = debug('brunch:watch');
...
async function loadConfigAndPaths(options) {
    ...
    if (cfg.server.run)
        promises.push(serve(cfg.server));
...
class BrunchWatcher {
    ...
    initWatcher(watchedPaths) {
        ...
        this.watcher = chokidar.watch(watchedPaths, Object.assign({
            ...
            .on('add', absPath => {
            if (isDebug)
                brunchdebug(`add ${absPath}`);
            const path = getRelativePath(paths.root, absPath);
            if (isConfig(path))
                return; // Pass for the initial files.
            this.startCompilation('change', path);
        })
            .on('change', absPath => {
            if (isDebug)
                brunchdebug(`change ${absPath}`);
            const path = getRelativePath(paths.root, absPath);
            ...
            .on('unlink', absPath => {
            if (isDebug)
                brunchdebug(`unlink ${absPath}`);
            const path = getRelativePath(paths.root, absPath);
            if (isConfig(path))
                return this.exitProcessFromFile(path);
    ...
    async restartBrunch(installPackages = true) {
        try {
            await this.close();
            if (installPackages)
                await installDeps(this.config.paths.root, { logger });
        }
        finally {
        ...
        if (process.send && process.env.BRUNCH_FORKED_PROCESS === 'true') {
            ...
            console.log('loading new watcher')
            newWatcher.init();
            console.log('new watcher loaded')
            if (this._onReload)
                this._onReload(newWatcher);
            return newWatcher;
        }
    }
    close() {
    ...
    compile() {
        ...
        write(fileList, config, joinConfig, optimizers, startTime).then(data => {
            ...
            logger.info(helpers.generateCompilationLog(startTime, assets, generatedFiles, disposed));
            ...
            if (error.code === 'WRITE_FAILED')
                return; // Ignore write errors as they are logged already
    ...
    _createProgress() {
        if (this._compilationProgress || process.env.DEBUG)
            return false;
        const passedTime = this._isFirstRun && speed.sinceStart;
        this._compilationProgress = helpers.getCompilationProgress(passedTime, logger.info);
    }
