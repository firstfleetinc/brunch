...
'use strict';
const brunchdebug = debug('brunch:file');
const readFromCache = { readFile }.readFile;
...
const getInitialType = path => {
    if (deppack.isNpmJSON(path) || path.endsWith('.js'))
        return 'javascript';
...
class SourceFile {
    ...
    async compile() {
        ...
        try {
            ...
            if (!exports || this.type === 'javascript')
                return;
    ...
    async _updateMap(compiled, sourceMap) {
        ...
        if (sourceMap) {
            brunchdebug(`Generated source map for '${path}'`);
        }
    ...
    dispose() {
        brunchdebug(`Disposing file ${this.path}`);
        this.path = '';
        this.source = '';
        this.disposed = true;
        this.error = undefined;
