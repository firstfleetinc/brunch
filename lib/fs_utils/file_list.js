...
import { updateCache } from "fcache";
import { formatError, frozenMap, frozenSet } from "../utils/helpers.js";
import Asset from "./asset.js";
import SourceFile from "./source_file.js";
import BrunchError from "../utils/error.js";
import anymatch from "anymatch";
'use strict';
const brunchdebug = debug('brunch:list');
const readAndCache = { updateCache }.updateCache;
...
class FileList extends EventEmitter {
    ...
    _compileDependencyParents(path) {
        ...
        if (!parents.length)
            return;
        brunchdebug(`Compiling ${isAsset ? 'asset' : 'dependency'} ${path} parent(s): ${paths.join(', ')}`);
