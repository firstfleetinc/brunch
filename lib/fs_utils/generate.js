...
import * as hmr from "../utils/hmr.js";
import BrunchError from "../utils/error.js";
'use strict';
const brunchdebug = debug('brunch:generate');
...
function concat(files, path, definitionFn, autoRequire, config) {
    ...
    brunchdebug(`Concatenating [${str}] => ${path}`);
...
function prepareSourceMap(optimizedMap, sourceFiles) {
    ...
    if (map._sourcesContents == null)
        map._sourcesContents = {};
...
const runOptimizer = optimizer => params => {
    ...
    brunchdebug(`Optimizing ${path} @ ${optimizer.brunchPluginName}`);
...
async function generate(path, targets, config, optimizers) {
    ...
    const cc = concat(sorted, path, definition, norm.modules.autoRequire[joinKey], config);
    const code = cc.code;
    const map = cc.map;
