...
const brunchdebug = debug('brunch:pipeline');
...
async function lint(file) {
    ...
    const linters = respondTo('lint').filter(l => l.pattern.test(path)).map(linter => {
        brunchdebug(`Linting ${path} @ ${linter.brunchPluginName}`);
...
async function getDependencies(file, compiler) {
    if (typeof compiler.getDependencies !== 'function')
        return [];
    brunchdebug(`Fetching dependencies ${file.path} @ ${compiler.brunchPluginName}`);
    try {
        let deps = compiler.getDependencies(file);
        if (!Array.isArray(deps) && deps instanceof Promise)
            deps = await deps;
...
async function compile(file, compiler, isStatic) {
    ...
    brunchdebug(`${op} ${path} @ ${compiler.brunchPluginName}`);
    try {
