...
import { checkDeps, installDeps } from "deps-install";
import { extname } from "path";
import sysPath from "universal-path";
import path from "path";
import anymatch from "anymatch";
import logger from "loggy";
import { deepAssign, fsExists, asyncFilter, isSymlink } from "./helpers.js";
import deppack from "deppack";
import * as wrappers from "./modules.js";
import loadPlugins from "./plugins.js";
import validateConfig from "./config-validate.js";
import BrunchError from "./error.js";
const __dirname = path.dirname(fileURLToPath(import.meta.url))
//import moduleExecutor from './module_executor.js';
const readFile = promisify(fs.readFile);
import {createRequire} from "module";
const requireUrl = createRequire(import.meta.url);
const worker = new Worker('./lib/utils/module_executor.mjs', {workerData: {test: 'data'}})
worker.once("message", result => {
  console.log(`${test}th Fibonacci Number: ${result}`);
});
//
...
//      const worker = new Worker(`${rootPath}/module_executor.js`, {
//        workerData: { modulePath: path }
//      })
...
export const applyOverrides = (config, env) => {
  ...
  env.forEach((override) => {
    ...
    const overrideProps =
      (config.overrides && config.overrides[override]) || {};
    ...
    Object.keys(specials).forEach((k) => {
      ...
      if (plug && plug[v]) {
        ...
        overrideProps.plugins[v] = item.concat(
          cItem.filter((plugin) => {
...
const normalizeJoinConfig = (joinTo) => {
  const object =
    typeof joinTo === "string" ? { [joinTo]: () => true } : joinTo || {};
...
const createJoinConfig = (cfgFiles, paths) => {
  ...
  types.forEach((type) => {
    ...
    if (type !== "javascripts") {
      logger.warn(
        `entryPoints can only be used with 'javascripts', not '${type}'`
      );
      return;
    }
    Object.keys(fileCfg.entryPoints).forEach((target) => {
      const isTargetWatched = paths.watched.some((path) =>
        target.startsWith(`${path}/`)
      );
      if (!isTargetWatched) {
        logger.error(
          `The correct use of entry points is: \`'entryFile.js': 'outputFile.js'\`. The entryFile ${target} is not inside of a watched directory, add directory path to the watched array`
        );
      }
      const entryCfg = fileCfg.entryPoints[target];
      const alreadyDefined = Object.keys(entryCfg).some(
        (out) => out in joinConfig[type]
      );
      if (alreadyDefined) {
        logger.warn(
          `config.files.${type}.joinTo is already defined for '${target}', can't add an entry point`
        );
        return;
      }
      ...
      Object.keys(normalizedEntryCfg).forEach((path) => {
        if (outPaths.includes(path)) {
          logger.warn(
            `'${path}' is already used by another entry point, can't add it to config.files.${type}.entryPoints for '${target}'`
          );
...
const setLoggyOptions = (config) => {
  ...
  if (config === true) {
    logger.warn(
      "`config.notifications: true` is deprecated. Notifications are on by default. Remove the option"
    );
    config = {};
  } else if (Array.isArray(config)) {
    logger.warn(
      "`config.notifications` array is deprecated. Use `config.notifications.levels` instead."
    );
...
const setConfigDefaults = (config, configPath) => {
  ...
  const join = (parent, name) => {
    return sysPath.isAbsolute(name)
      ? name
...
const normalizeConfig = (config) => {
  ...
  const maybeConfigs = ['.js', '.json', '.node'].map(
    (ext) => paths.config + ext
  );
  ...
  Object.keys(config.conventions).forEach((key) => {
    ...
    conventions[key] =
      key === "vendor" ? fn : (path) => !deppack.isNpm(path) && fn(path);
  });
  console.log('allconfig files:', allConfigFiles)
  console.log('important paths', paths)
  config._normalized = {
...
const addDefaultServer = async (config) => {
  if (config.server.path) return;
  try {
    const defaultServerFilename = "brunch-server";
    const resolved = await import(`${pathToFileURL(sysPath.resolve(defaultServerFilename))}?id=${Math.random().toString(36).substring(3)}`);
    try {
    } catch (error) {
      // Do nothing.
    }
...
const addPackageManagers = async (config) => {
  try {
    ...
    const jsonPath = sysPath.resolve(rootPath, packageConfig);
      const json = requireUrl(jsonPath);
...
async function checkProjectDependencies(config) {
  ...
  logger.info(
    `Using outdated versions of ${unmet.join(
      ", "
    )}, trying to update to match package.json versions`
  );
...
async function tryToLoad(root, configPath, pkg) {
  ...
  const fullConfigPath = sysPath.isAbsolute(bname)
    ? bname
    : sysPath.join(root, bname);
  let brunchConfig;
  ...
  if (pkg.brunch) {
    ...
    const fullPath = sysPath.resolve(fullConfigPath);
    const importPath = pathToFileURL(fullPath);
    worker.postMessage({modulePath: importPath});
    //await runWorker(importPath);
    //const fullPath = require.resolve(sysPath.resolve(fullConfigPath));
    //delete require.cache[fullPath];
    let obj = undefined;
    try {
        console.log('import path', `${importPath.href}?id=${Math.random().toString(36).substring(3)}`);

      obj = await import(`${importPath.href}?id=${Math.random().toString(36).substring(3)}`)
      ...
      if (configDep in deps) {
        logger.warn(
          `brunch-config requires '${configDep}' which is in package.json but wasn't yet installed. Trying to install...`
        );
...
console.log('final config', config)
    ...
    if (!config.files) {
      throw new BrunchError("CFG_NO_FILES", {
        basename,
        minimalConfig: DEFAULT_CONFIG_CONTENTS,
      });
    }
    ...
    asyncFilter(["config.coffee", "brunch-config.coffee"], (file) => {
      ...
      if (list.length) {
        logger.error(
          "brunch-config with coffeescript is no longer supported. Compile it to JS: \n\nnpm install --global coffeescript\ncoffee --bare --compile brunch-config.coffee"
        );
      }
    });
  }
...
export async function loadConfig(options) {
  ...
  if (!fromWorker) await addDefaultServer(config);
  ...
  if (config.npm.checkDependencies) {
...
export async function loadConfigAndPlugins(options) {
