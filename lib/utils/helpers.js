import { basename as basename$0 } from "universal-path";
import { promisify } from "util";
import fs from "fs";
'use strict';
const basename = { basename: basename$0 }.basename;
...
function generateCompilationLog(startTime, copiedAssets, generatedFiles, disposedFiles) {
    ...
    generatedFiles.forEach(generatedFile => {
        ...
        generatedFile.sourceFiles.forEach(sourceFile => {
            ...
            if (!isChanged && dgen.includes(generatedFile))
                isChanged = true;
        });
        if (isChanged) {
    ...
    const cachedLog = (() => {
        if (cachedCount === 0)
            return compiled.length <= 1 ? '' : ' files';
        switch (compiled.length) {
            case 0:
...
export { prettify };
export { formatError };
export { formatOptimizerError };
export { generateCompilationLog };
export { getCompilationProgress };
export { deepFreeze };
export { fsExists };
export { isSymlink };
export { frozenMap };
