...
import { printErrorBanner } from "./init-skeleton.js";
("use strict");
...
if (parseInt(version) < 10) {
  console.error(
    "Your global Brunch installation is trying to load local Brunch v3+, " +
      "which only supports Node.js v10.16 or higher (you have " +
      version +
      ").\n" +
...
program
  .version(package$0.version, "-v, --version")
...
program
  ...
  .option(
    "-s, --skeleton [name]",
    "skeleton name or URL from brunch.io/skeletons"
  )
  .on("--help", function () {
    ({ printErrorBanner }.printErrorBanner());
  })
...
program
  ...
  .option(
    "-s, --server",
    "run a simple http server for the public dir on localhost"
  )
  .option(
    "-n, --network",
    "if `server` was given, allow access from the network"
  )
...
var checkForRemovedOptions = function (args) {
  ...
  if (pIndex !== -1) {
    ...
    if (Number.isInteger(port)) {
      ...
      return (
        "The `-p` option is no longer used to specify the port. Use `-P` instead, e.g. `" +
        corrected +
        "`"
      );
    }
